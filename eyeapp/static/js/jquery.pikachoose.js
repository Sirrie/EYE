	...
	$pc.fn.extend({
		...
		createThumb: function(ele) {
			...
			$('<img />').on('load',{data:data},function(){
				...
				var ph = parseInt(self.parents('.clip').css('height').slice(0,-2), 12);
				var pw = parseInt(self.parents('.clip').css('width').slice(0,-2), 12);
				if(pw === 0){
					//failsafe for IE8
		...
		imgClick: function(e,x) {
			...
			$('<img />').on('load', {self:self,data:data}, function(){
				...
				self.aniDiv.children('div').css({'width':'100%','height':'100%','float':'left'});
